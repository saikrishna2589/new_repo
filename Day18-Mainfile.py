...
#standard library.this helps to check if file exists or not. ex-if t0do.txt doesn't exist, os.path.exists("t0do.txt)
# vaue is False. If exists, True. we can ma
import os

#so if the file doesn't exist when passing the program on to user , then it creates one for them for the 1st run.
#if the file exists, it does not overwrite as the conditional block is not applicable then

#in the terminal , to generate exec files type(we need to run in command line i.e terminal)
# pyinstaller --onefile --windowed --clean Day18-Creating Standalone executable program.py
